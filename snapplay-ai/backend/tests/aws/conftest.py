...
def standard_queue(sqs: Any) -> str:
    return sqs.create_queue(QueueName="snapplay-jobs", Attributes={"VisibilityTimeout": "60"})[
        "QueueUrl"
    ]
...
def queue_with_dlq(sqs: Any) -> tuple[str, str]:
    ...
    dlq_url = sqs.create_queue(QueueName="snapplay-jobs-dlq")["QueueUrl"]
    dlq_arn = sqs.get_queue_attributes(QueueUrl=dlq_url, AttributeNames=["QueueArn"])[
        "Attributes"
    ]["QueueArn"]
    queue_url = sqs.create_queue(
        QueueName="snapplay-jobs-redrive",
        Attributes={
            "VisibilityTimeout": "0",
