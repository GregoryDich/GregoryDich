...
otherwise. Storage follows ``STORAGE_BACKEND``; dispatch follows ``SNAPPLAY_PIPELINE``.
...
def events_mode(settings: Settings) -> EventsMode:
    ...
    return "bus" if settings.snapplay_pipeline in LOCAL_PIPELINES else "poll"
...
def build_dispatch(
    ...
    pipeline = settings.snapplay_pipeline
