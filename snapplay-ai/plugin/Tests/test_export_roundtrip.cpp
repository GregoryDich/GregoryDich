...
/*
 * Exercises both writers on one fixture. When SNAPPLAY_TEST_OUT names a directory, the
...
std::string fixtureJson (const std::vector<Track>& tracks)
{
    ...
    json << "{\n  \"bpm\": " << fixtureBpm << ",\n  \"midi_ppq\": 480,\n  \"fsc_ppq\": 96,\n"
         << "  \"fl_version\": \"" << snapplay::core::fscVersionText << "\",\n  \"tracks\": [\n";
...
SNAPPLAY_TEST(exportFixtureWritesBothFormats)
{
    const auto tracks = makeFixture();
    const auto midi = snapplay::core::writeMidiFile (tracks, fixtureBpm);
    const auto fsc = snapplay::core::writeFsc (tracks, fixtureBpm);

    SNAPPLAY_CHECK (midi.size() > 14 && std::string (midi.begin(), midi.begin() + 4) == "MThd");
    SNAPPLAY_CHECK (fsc.size() > 22 && std::string (fsc.begin(), fsc.begin() + 4) == "FLhd");

    const char* outDir = std::getenv ("SNAPPLAY_TEST_OUT");
    ...
    SNAPPLAY_CHECK (! error);

    SNAPPLAY_CHECK (writeFile (directory / "sample.mid", midi));
    SNAPPLAY_CHECK (writeFile (directory / "sample.fsc", fsc));
    SNAPPLAY_CHECK (writeFile (directory / "fixture.json", fixtureJson (tracks)));
}
