"""Shared fixtures: isolated environment, respx routers and SnapPlay fixtures.
...
SNAPPLAY_URL = "https://api.snapplay.test"
STORAGE_URL = "https://storage.snapplay.test"
...
def mock_snapplay(
    ...
    routes = {
        ...
        "poll": router.get(f"{SNAPPLAY_URL}/v1/jobs/{result['job_id']}").mock(
    ...
    if sse:
        routes["events"] = router.get(f"{SNAPPLAY_URL}/v1/jobs/{result['job_id']}/events").mock(
            return_value=httpx.Response(
                200,
...
def env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, str]:
    values = {
        "SNAPPLAY_API_URL": SNAPPLAY_URL,
        "SNAPPLAY_API_KEY": "sp_live_testtesttesttesttesttesttest",
        "SNAPPLAY_POLL_INTERVAL_SECONDS": "0",
        "SNAPPLAY_JOB_TIMEOUT_SECONDS": "5",
