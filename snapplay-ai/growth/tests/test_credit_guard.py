...
from tests.conftest import SNAPPLAY_URL, make_job_result, make_wav_bytes, me_body, mock_snapplay
...
async def test_balance_floor_stops_the_run_before_the_first_clip(
    ...
    routes = mock_snapplay(router, make_job_result(), available=41)
...
async def test_per_run_credit_budget_is_honoured(
    ...
    routes = mock_snapplay(router, make_job_result(), available=1000)
    report = (
...
async def test_the_guard_reads_the_balance_with_the_api_key(
    ...
    assert request.url == f"{SNAPPLAY_URL}/v1/me"
    assert request.headers["X-API-Key"].startswith("sp_live_")
