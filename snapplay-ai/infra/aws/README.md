# SnapPlay AI — AWS deployment (contract §10)
...
STATE_BUCKET="snapplay-terraform-state-${ACCOUNT_ID}"
...
aws dynamodb create-table --table-name snapplay-terraform-locks \
...
Run from `snapplay-ai/infra/aws` with administrator credentials.
...
2. **Create the registries first** so the images can be pushed before the ECS
   ...
   ```
3. **Build and push both images** (context is `snapplay-ai/`):
   ```bash
   REGISTRY=$(aws sts get-caller-identity --query Account --output text).dkr.ecr.$AWS_REGION.amazonaws.com
   aws ecr get-login-password | docker login --username AWS --password-stdin "$REGISTRY"
   cd ../..   # snapplay-ai/
   docker build -f infra/Dockerfile.api    -t "$REGISTRY/snapplay-prod/api:latest"    .
   docker build -f infra/Dockerfile.worker -t "$REGISTRY/snapplay-prod/worker:latest" .
   docker push "$REGISTRY/snapplay-prod/api:latest"
   docker push "$REGISTRY/snapplay-prod/worker:latest"
   cd infra/aws
   ```
...
| `project_name` / `environment` | `snapplay` / `prod` | resource name prefix `snapplay-prod` |
...
`snapplay-prod-*` roles/policies, which is what `terraform apply` needs.
...
task/execution roles and security group and logs to `/ecs/snapplay-prod/api`. The
...
* Scale out: the `snapplay-prod-worker-scale-out` alarm fires when
  `ApproximateNumberOfMessagesVisible >= 1` and adds 1 task (2 when 5+ messages).
* Scale in: `snapplay-prod-worker-scale-in` fires after 10 minutes with
