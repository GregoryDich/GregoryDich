...
виде, в каком они существуют в коде сегодня* (`TONAMORPH_PIPELINE`, `tonamorph-worker`,
`com.tonamorph.plugin`, префикс `tonamorph-prod` в Terraform). Репозиторий уже переименован
в Tonamorph, идентификаторы окончательные — копируй как есть, чтобы всё совпадало с кодом.

**Плейсхолдеры.** `<domain>` — домен из решения №1 (`tonamorph.com`);
...
|---|---------|----------|------------------|---------|---------------|
| 1 | **Название и домен** | решено: **Tonamorph** (три нейминг-спринта, 90+ проверок доменов, проверка конфликтов; запасной Parsetone) | `tonamorph.com` регистрирует основатель сам; марка USPTO с первой прибыли | `tonamorph.com` $11.25/год; `.io` $30/год; `.ai` $160/2 года (цены Vercel) | Домен нужен для Vercel, Supabase Site URL и писем. Коды плагина `Tona`/`Mrph` и `com.tonamorph.plugin` после первого релиза менять нельзя. |
...
| 4 | **Rubber Band** | купить коммерческую лицензию · собирать с `TONAMORPH_USE_RUBBERBAND=OFF` | **OFF на запуск**, купить, когда появится выручка | лицензия — разовая, бессрочная, без роялти; цена не публикуется, нужен запрос на breakfastquay.com | Release-сборку. GPL-вариант в закрытом плагине распространять нельзя. С `OFF` работает резервный ресемплинг (Lagrange): длительность стема меняется на `2^(-semitones/12)` — хуже, но легально. |
...
### 1.1 Название — почему Tonamorph

Решение принято 2026-09-11 после трёх параллельных нейминг-спринтов (метафоры, коинованные
слова, глаголы-действия), 90+ проверок доменов и проверки конфликтов по каждому финалисту.
Матрица оценки: смысл 25, одно чтение 20, форма 15, образ 15, право 10, владение 10,
растяжимость 5.

* **Tonamorph** (91/100) — TOH-na-morf, «тон трансформируется в инструмент»; читается
  одинаково в EN/RU/DE/ES/HE; три слога, девять букв; продуктов, артистов и хэндлов не
  найдено; `.com`, `.io`, `.ai` были свободны. Как выдуманное слово регистрируется в USPTO
  сильнее всех кандидатов (нет спора об описательности). Риск для проверки: марки с корнем
  MORPH в классах 9/42 (Zynaptiq Morph, FL Morphine) — разные слова, но проверить в TSDR.
* **Parsetone** (83) — запасной: точный, инженерный; `.com` и `.ai` свободны; холоднее для
  продюсера.
* Отвергнуты: StemKeys (не премиально, «stem» — самое конкурентное слово категории);
  Rechord (живая марка reChord в классе 9, шесть музыкальных продуктов, автокоррекция в
  «record»); Resynth (описательный DSP-термин, марки RESYN рядом, привязка к Samplab);
  Tonestem (двусмысленность «стем/стам», TONESTACK рядом); Retone (шесть действующих
  компаний, в том числе слуховые аппараты); Partone («part one», Dolly Parton);
  NoteBreak (занято музыкантом с аутро у Mark Rober); TrackBreak (термин в аудио-софте).

Базы USPTO/EUIPO/WIPO из среды разработки недоступны — только поисковые сниппеты. Перед
подачей марки: TSDR и TMview по TONAMORPH и MORPH в классах 9 и 42 (раздел 4, юрист).

**Ты:** регистрируешь `tonamorph.com` и сообщаешь мне — дальше DNS, Vercel и Supabase.
...
- [ ] **Купить домен** ⏱ 5 мин 💳 см. таблицу в разделе 1
  ...
  2. Минимальный набор: `tonamorph.com` (регистрируешь сам). `.io` и `.ai` — по желанию,
     для защиты (редиректы на `.com`), можно после первой прибыли.
...
- [ ] **pg_cron и очистка** ⏱ 3 мин
  ...
  3. ✅ `select jobname, schedule from cron.job;` → `tonamorph_expire_credits`,
     `tonamorph_reap_stale_jobs`, `tonamorph_purge_expired_jobs`. Второй — страховка от
...
- [ ] **Записать price id в базу** ⏱ 3 мин — Supabase → SQL Editor:
  ...
  ✅ На сервере: `cd tonamorph/backend && python3 -m app.checks` → `"ok": true`, код
  выхода 0.
...
- [ ] **Проверка результата релиза** (раздел 3, день 2)
  ...
  ✅ `auval -v aumu Mrph Tona` (коды из `plugin/CMakeLists.txt`) → `AU VALIDATION SUCCEEDED`. Если Logic всё равно не видит
...
- [ ] **Секрет для воркера** ⏱ 5 мин
  1. Dashboard → **Secrets → Create new secret → Custom** → Name ровно
     **`tonamorph-supabase`** (значение по умолчанию `TONAMORPH_MODAL_SECRETS` в
     `backend/worker/modal_app.py`).
  2. Ключи: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `STORAGE_BACKEND=supabase`,
     `STORAGE_BUCKET=jobs`, `TONAMORPH_PIPELINE=local`, `ENV=staging` (валидация
...
- [ ] **Деплой воркера и бенчмарк** ⏱ 10 мин (после смены движка разделения — решение №2)
  ...
  TONAMORPH_MODAL_GPU=A10G modal deploy -m worker.modal_app
  ```
  ✅ Dashboard → **Apps → `tonamorph-worker`** → класс воркера (`CLASS_NAME` из `backend/worker/modal_app.py`), статус Deployed.
  ...
  🔑 В backend `.env`: `TONAMORPH_PIPELINE=modal`, `MODAL_APP_NAME=tonamorph-worker`.
...
- [ ] **Сеть** ⏱ 10 мин
  ...
  5. 🔑 `https://api.<domain>` → `NEXT_PUBLIC_API_URL`, `TONAMORPH_API_URL` (growth), и
...
- [ ] **Конфиг API на сервере** ⏱ 10 мин
  1. `git clone` репозитория на сервер (deploy key read-only — сделаю и пришлю), файл
     `tonamorph/backend/.env` с правами `chmod 600`. Значения:
     ...
     | `TONAMORPH_PIPELINE=modal`, `MODAL_APP_NAME=tonamorph-worker` | 2.7 |
  ...
  3. `cd tonamorph/infra && docker compose up -d api` (после моего PR — вместе с Caddy).
...
- [ ] **Проект из репозитория** ⏱ 10 мин 💳 Hobby $0 (для коммерческого сайта Vercel
  ...
  2. **Root Directory** → Edit → `tonamorph/web`. Framework Preset — Next.js определится сам.
...
- [ ] ⏱ 5 мин 💳 $0 (Developer plan: 5k ошибок/мес)
  ...
  3. 🔑 В backend `.env` как `SENTRY_DSN` (уже в `config.py`; пустое значение = выключено).
     ...
     `TF_VAR_sentry_dsn`), для Modal — та же переменная в секрете `tonamorph-supabase`.
...
**Я:** переименование репозитория целиком (сделано, все сьюты зелёные) → перенос в новый репозиторий → правка `production.tfvars`
...
6. **Торговая марка.** Стоит ли подать на `<Имя>` в классах 9 и 42 (Израиль — ILPO,
   ...
   проверка TONAMORPH и марок с корнем MORPH (Zynaptiq Morph, Image-Line Morphine) в классах 9/42.
...
| 9 | Марка TONAMORPH и марки с корнем MORPH в классах 9/42 (США, ЕС, Израиль) | только поисковые сниппеты, базы USPTO/EUIPO/WIPO не открывались | `tmsearch.uspto.gov`, `euipo.europa.eu/eSearch`, `branddb.wipo.int`; лучше — платный клиренс через юриста |
| 10 | Соцхэндлы `@tonamorph` на IG/TikTok/X/YouTube | «нет следов», не «свободно» | открой профили руками, зарегистрируй в день выбора имени |
