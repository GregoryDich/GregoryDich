...
|---|---------|----------|------------------|---------|---------------|
| 1 | **Название и домен** | StemKeys · NoteBreak · TrackBreak (см. ниже) | **StemKeys** + `stemkeys.com` | `stemkeys.com` $11.25/год; `stemkeys.app` $9.99/год; `stemkeys.io` $30/год; `stemkeys.ai` $160/2 года; `notebreak.ai` $160/2 года (цены Vercel) | **Всё.** Идентификаторы плагина (`BUNDLE_ID`, коды `Tona`/`Mrph`) после первого релиза менять нельзя — DAW-проекты ссылаются на них навсегда. Переименование идёт первым. |
...
Прежнее кодовое имя отвергнуто и нигде дальше в этом документе не используется как
название продукта; репозиторий переименован в **Tonamorph** (см. `ROADMAP.md`, шаг 1).
