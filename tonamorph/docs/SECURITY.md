...
| Growth engine (`growth/mcp_server`) | one `tm_live_…` API key (§11) | `/v1/*` as the key's owner | user JWTs, anything not reachable with a key |
...
* **Token handling in the plugin.** The password is never stored (§1), and no token ever
  ...
  (`Tonamorph.settings` under a `Tonamorph` folder in the OS application-data location),
...
* **Format.** `tm_live_` + 32 hex characters from a CSPRNG (`secrets.token_hex(16)` —
...
| **API-key theft** | leaked `tm_live_…` from CI or the growth engine | `DELETE /v1/api-keys/{id}` revokes instantly; damage capped by the owner's balance and rate limits; keys never appear in logs; the growth engine loads it from the environment only | §11 |
