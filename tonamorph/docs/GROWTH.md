...
`growth/` is an automation engine that produces short-form videos showing Tonamorph
...
The server (`FastMCP("tonamorph-growth")`) authenticates to the API with
`TONAMORPH_API_KEY` from the environment (§11); it never handles user JWTs. Signatures
...
* **Campaign attribution.** `mcp_server/attribution.py` builds every post's destination link,
  ...
  ```
  https://tonamorph.com/?utm_source=tiktok&utm_medium=social&utm_campaign=ugc_shorts
