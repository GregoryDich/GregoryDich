...
TonamorphLookAndFeel::TonamorphLookAndFeel()
...
juce::Font TonamorphLookAndFeel::font (float height, bool bold)
{
    const auto options = juce::FontOptions (height);
    return juce::Font (bold ? options.withStyle ("Bold") : options);
}

void TonamorphLookAndFeel::drawRotarySlider (juce::Graphics& g, int x, int y, int width, int height,
...
void TonamorphLookAndFeel::drawButtonBackground (juce::Graphics& g, juce::Button& button,
...
juce::Font TonamorphLookAndFeel::getTextButtonFont (juce::TextButton&, int buttonHeight)
{
    return font (juce::jmin (15.0f, static_cast<float> (buttonHeight) * 0.6f));
}

juce::Font TonamorphLookAndFeel::getComboBoxFont (juce::ComboBox&)
{
    return font (14.0f);
}
