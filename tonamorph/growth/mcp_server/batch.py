...
from .tonamorph import TonamorphApiError, TonamorphClient
...
class CreditGuard:
    ...
    async def _available(self) -> int:
        client = TonamorphClient(
            self.settings.tonamorph_api_url,
            self.settings.tonamorph_api_key or "",
            http=self.http,
            poll_interval=self.settings.tonamorph_poll_interval_seconds,
            timeout=self.settings.tonamorph_job_timeout_seconds,
        )
    ...
    async def blocker(self) -> str | None:
        ...
        if not self.settings.tonamorph_api_key:
            return None
        try:
            available = await self._available()
        except (TonamorphApiError, httpx.HTTPError) as exc:
