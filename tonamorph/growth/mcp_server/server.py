"""The ``tonamorph-growth`` MCP server.
...
INSTRUCTIONS = (
    "Tonamorph growth engine. Turns rights-cleared audio clips into 15-second vertical shorts "
    "(Tonamorph stem/MIDI processing → brief → voiceover → Remotion render) and publishes them. "
...
mcp = FastMCP("tonamorph-growth", instructions=INSTRUCTIONS)
...
async def process_clip(
    ...
    """Submit a rights-cleared clip to the Tonamorph API and pick the most usable stem.
    ...
    X-API-Key from TONAMORPH_API_KEY, follows ``GET /v1/jobs/{id}/events`` (SSE) to the result
...
async def render_video(
    ...
    Args:
        ...
        scene: Top half: a stylised Tonamorph plugin UI or a DAW arrangement view.
