"""Tonamorph API client (contract §2 jobs, §5 errors, §11 API keys).
...
class TonamorphApiError(RuntimeError):
    def __init__(
        self,
        status: int,
        code: str,
...
def _raise_for_error(response: httpx.Response) -> None:
    ...
    try:
        ...
        pass
    raise TonamorphApiError(response.status_code, code, message, details)
...
class TonamorphClient:
    ...
    async def __aenter__(self) -> TonamorphClient:
        return self
                ...
                async for line in response.aiter_lines():
                    if line.strip() == "":
                        ...
                        if event == "error":
                            ...
                            raise TonamorphApiError(200, err.code, err.message)
                        continue
