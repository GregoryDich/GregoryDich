...
from tests.conftest import JOB_ID, TONAMORPH_URL, make_job_result, make_wav_bytes, mock_tonamorph
...
async def test_process_clip_from_url_uses_polling_fallback(
    ...
    routes = mock_tonamorph(router, make_job_result(), sse=False)
...
async def test_process_clip_requires_api_key(
    ...
    monkeypatch.delenv("TONAMORPH_API_KEY")
    with pytest.raises(ToolError, match="TONAMORPH_API_KEY"):
        await mcp_client.call_tool("process_clip", {"clip_path_or_url": str(clip_file)})
