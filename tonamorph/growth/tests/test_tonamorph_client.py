"""Tonamorph API client: multipart submit, SSE follow, polling fallback and error mapping."""
...
from mcp_server.tonamorph import JobOptions, TonamorphApiError, TonamorphClient, idempotency_key_for
...
def _client() -> TonamorphClient:
    return TonamorphClient(TONAMORPH_URL, "tm_live_key", poll_interval=0, timeout=5, max_retries=2)


async def test_submit_sends_multipart_with_api_key(router: respx.Router) -> None:
    result = make_job_result()
    routes = mock_tonamorph(router, result)
    ...
    assert request.headers["X-API-Key"] == "tm_live_key"
...
async def test_polling_fallback_when_stream_unavailable(router: respx.Router) -> None:
    result = make_job_result()
    routes = mock_tonamorph(router, result, sse=False)
...
async def test_sse_error_event_raises(router: respx.Router) -> None:
    ...
    async with _client() as client:
        with pytest.raises(TonamorphApiError) as exc:
...
async def test_failed_job_raises_with_error_code(router: respx.Router) -> None:
    ...
    async with _client() as client:
        with pytest.raises(TonamorphApiError, match="worker_timeout"):
            await client.wait_for_result(JOB_ID)


async def test_contract_error_envelope_is_mapped(router: respx.Router) -> None:
    router.post(f"{TONAMORPH_URL}/v1/jobs").mock(
        return_value=httpx.Response(
            402,
            json={
                "error": {
...
async def test_rate_limit_is_retried_after_retry_after(router: respx.Router) -> None:
    route = router.post(f"{TONAMORPH_URL}/v1/jobs")
    route.side_effect = [
        httpx.Response(
            429,
