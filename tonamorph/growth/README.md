# Tonamorph growth engine

An MCP server (`tonamorph-growth`) that turns rights-cleared audio clips into 15-second vertical
shorts and publishes them: Tonamorph API job → best stem + MIDI → scripted brief → ElevenLabs
...
* **Tonamorph API:** clips are capped at 10 MB / 60 s and each job costs one credit (contract
...
| Variable | Purpose |
|---|---|
| `TONAMORPH_API_URL`, `TONAMORPH_API_KEY` | Tonamorph API base URL and `tm_live_…` key sent as `X-API-Key` |
...
```json
{
  "mcpServers": {
    "tonamorph-growth": {
      "command": "python3",
      "args": ["-m", "mcp_server"],
      "cwd": "/path/to/tonamorph/growth",
      "env": {
        "TONAMORPH_API_KEY": "tm_live_…",
        "LICENSED_CLIPS_DIR": "/path/to/licensed_clips",
        "REMOTION_DIR": "/path/to/tonamorph/growth/remotion",
        "ELEVENLABS_API_KEY": "…"
      }
    }
  }
