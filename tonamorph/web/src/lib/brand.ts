/**
 * Every product-facing name, address and URL comes from here so that a rename stays a
 * one-line change: no page, component or metadata may contain the name as a literal —
 * always `brand.productName`. All values are NEXT_PUBLIC_* so client components can
 * read them too; none of them are secrets.
 */

const DEFAULT_PRODUCT_NAME = "Tonamorph";
...
const siteUrl = stripTrailingSlash(text(raw.siteUrl, "https://tonamorph.com"));
...
export const brand = {
  ...
  slug: productName
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, ""),
  tagline: "Any track. Now an instrument.",
  siteUrl,
  siteHost,
  supportEmail,
  privacyEmail: text(raw.privacyEmail, supportEmail),
  legalEmail: text(raw.legalEmail, supportEmail),
  companyLegalName: text(raw.companyLegalName, "Tonamorph Audio"),
