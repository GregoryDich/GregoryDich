-- Tonamorph — 0001: extensions, enums, tables, triggers, seed data.
...
-- ---------------------------------------------------------------------------
-- API keys (§11): sha256 hex of "tm_live_<32 hex chars>", prefix kept for display
-- ---------------------------------------------------------------------------
...
values
  ...
  ('pack_50', '50 Credits', 50, 900, 'USD', null,
   'https://tonamorph.lemonsqueezy.com/checkout/buy/{variant_id}?checkout[custom][user_id]={user_id}&checkout[custom][ref]={ref}', 1),
  ('sub_monthly', 'Pro Monthly', 60, 799, 'USD', 'month',
   'https://tonamorph.lemonsqueezy.com/checkout/buy/{variant_id}?checkout[custom][user_id]={user_id}&checkout[custom][ref]={ref}', 2)
