...
FastAPI control plane for the Tonamorph plugin. The binding API is
...
app/
  ...
  auth/            jwt.py (Supabase token verification), api_keys.py (tm_live_ format + sha256)
...
* `X-API-Key: tm_live_<32 hex>` — the key is hashed with SHA-256 and resolved through
...
| dispatch | `TONAMORPH_PIPELINE` | `fake`/`local` in-process, `modal`, `runpod`, `aws` (SQS) |
| job events | `TONAMORPH_PIPELINE` | in-process bus for `fake`/`local`, DB polling for remote workers |
...
`get_pipeline(settings)` imports `app.pipeline.<TONAMORPH_PIPELINE>` and returns its
