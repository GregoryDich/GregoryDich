...
log = logging.getLogger("tonamorph.worker")
...
def load_pipeline(settings: Settings) -> RunPipeline:
    """``run_pipeline`` for ``TONAMORPH_PIPELINE``. The in-process GPU pipeline is refused
    ...
    if settings.tonamorph_pipeline == "local":
        separation.ensure_licensed(settings)
    run = get_pipeline(settings)
    if settings.tonamorph_pipeline == "fake":
        log.warning("TONAMORPH_PIPELINE=fake: this worker runs the deterministic CPU stub")
    return run
...
def warm_up_pipeline(settings: Settings) -> None:
    ...
    module = import_module(f"app.pipeline.{settings.tonamorph_pipeline}")
