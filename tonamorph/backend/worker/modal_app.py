"""Modal deployment of the GPU pipeline (contract §7, ``TONAMORPH_PIPELINE=modal``).
...
named ``tonamorph-worker``; the API calls
``modal.Cls.from_name("tonamorph-worker", "TonamorphWorker")().run``.

Configuration (read at deploy time): ``TONAMORPH_MODAL_GPU`` (default ``A10G``),
``TONAMORPH_MODAL_MIN_CONTAINERS`` (default ``0``), ``TONAMORPH_MODAL_SECRETS``
(comma-separated Modal secret names, default ``tonamorph-supabase``). Every ``modal``
...
APP_NAME = "tonamorph-worker"
CLASS_NAME = "TonamorphWorker"
DEFAULT_GPU = "A10G"
TIMEOUT_SECONDS = 60
CUDA_IMAGE = "nvidia/cuda:12.4.1-cudnn-runtime-ubuntu22.04"
BASIC_PITCH_PIN = "basic-pitch==0.4.0"
MODEL_CACHE_VOLUME = "tonamorph-model-cache"
MODEL_CACHE_DIR = "/root/.cache"
BACKEND_DIR = Path(__file__).resolve().parents[1]


def _gpu() -> str:
    return os.environ.get("TONAMORPH_MODAL_GPU", "").strip() or DEFAULT_GPU


def _min_containers() -> int:
    return max(0, int(os.environ.get("TONAMORPH_MODAL_MIN_CONTAINERS", "0") or 0))


def _secret_names() -> list[str]:
    raw = os.environ.get("TONAMORPH_MODAL_SECRETS", "tonamorph-supabase")
...
def build_app() -> tuple[Any, type]:
    ...
    TonamorphWorker.__name__ = CLASS_NAME
    return app, TonamorphWorker


if find_spec("modal") is not None:
    app, TonamorphWorker = build_app()
else:
    app = None
    TonamorphWorker = None
