...
log = logging.getLogger("tonamorph.pipeline.transcription")
...
def load_model() -> LoadedTranscriber:
    ...
    with _lock:
        ...
        path = os.environ.get("TONAMORPH_BASIC_PITCH_MODEL", "").strip() or str(default_path)
...
def _temp_wav(mono: np.ndarray, sample_rate: int) -> Iterator[str]:
    ...
    fd, path = tempfile.mkstemp(suffix=".wav", prefix="tonamorph-", dir=directory)
    os.close(fd)
    try:
