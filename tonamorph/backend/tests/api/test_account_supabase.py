...
async def test_api_key_listing_never_selects_the_hash(supabase: SupabaseClient) -> None:
    ...
    assert [k.prefix for k in listed] == ["tm_live_ab12"]
    params = route.calls.last.request.url.params
...
async def test_export_reads_every_table_and_pages_past_the_row_cap(
    ...
    assert export.api_keys[0].prefix == "tm_live_ab12"
    ...
    assert "payload" not in dumped and "customer_email" not in dumped and "tm_live_ab12" in dumped
