...
async def test_modal_dispatch_without_the_sdk_is_503(settings: Settings) -> None:
    dispatch = ModalDispatch(settings.model_copy(update={"tonamorph_pipeline": "modal"}))
...
def test_backend_selection_follows_settings(settings: Settings) -> None:
    ...
    assert events_mode(settings.model_copy(update={"tonamorph_pipeline": "aws"})) == "poll"
    ...
    modal = build_dispatch(
        settings.model_copy(update={"tonamorph_pipeline": "modal"}),
        jobs=services.jobs,
